...
map.on('click', function (e) {
    ...
    var popup = new mapboxgl.Popup().setLngLat(feature.geometry.coordinates).setHTML(popupContent).addTo(map);
});
...
$("#search").submit(function (event) {
    ...
    $('#layers-dropdown').val("default");
    // reset the dropdown to default value
    $("#searchText").val("");
    // empty the searchbox after choosing points
...
function processLayer(result) {
    ...
    if (layers[selection_label] !== undefined) {
        ...
        slider.addEventListener('input', function (e) {
            ...
            map.setPaintProperty(selection_label, 'raster-opacity', parseInt(e.target.value, 10) / 100);
            // Value indicator
